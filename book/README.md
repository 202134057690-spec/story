...
├── test_typeset.py فحصُ المحرّك (لا pytest؛ ./.venv/bin/python book/test_typeset.py)
├── art/            الرسوم مصدرًا — ملتقَطة، فالتوليدُ ليس حتميًا
├── fonts/          أميري (OFL) — لا تنزيلٌ وقت البناء
├── edition/        الناتج: PDF · معاينات · qa.json
└── README.md      هذا الملف
```
...
الخطّ والرسوم محفوظان في المستودع، فلا تنزيل وقت البناء. يبقى ما يُثبَّت بـ`pip`،
...
```bash
make test-book                    # المحرّك: bidi، رصّ القطع، الأرقام، الضبط، الورق
make check-full                   # + طبعةٌ كاملةٌ في مؤقّت ومقارنةُ القياسات
python3 tools/check_qa.py         # qa.json محفوظةُ الصدق
```

`book/test_typeset.py` لا يُجامل: يفحصُ أن الفاصلة تقع يسار الكلمة لا يمينها،
وأن `١٤` لا تُطبع `٤١`، وأن اسمًا لاتينيًّا في حاشيةٍ عربيّةٍ لا ينعكس،
وأن صفحة المعاينة بأبعاد A5 (داءُ لوحةٍ بحجم ٢×٢ بكسل)، وأن لا يتيمةَ كلمةٍ
في آخر الفقرة. هذه كلُّها كُسِرَت مرةً على الأقلّ في هذا المستودع.
...
| `--dpi-preview N` | دقّة صور المعاينة (لا تمسّ الـ`PDF`) |
